...
void yolo(void){	// Build an array of one microcode triad (3 uops+ seq word)
	...
	patch_ucode(0x7c22,ucode_patch,ARRAY_SZ(ucode_patch));
	//Writes the patch into ucode RAM at addresses starting U7AD0
	hook_match_and_patch(0, 0x0740, 0x7c22);
...
int main(){
	uint64_t rax, rbx;
	yolo();
	asm("sysexitq" : "=a" (rax), "=b" (rbx));
	//EXECute a SYSEXITQ to trigger the oatched oatch; capture the resulting	//RAX/RBX outputs back into C variables
	printf("a=%lx b=%lx\n", rax, rbx);
}
