...
void yolo(void){
	// Build an array of one microcode triad (3 uops+ seq word)
	ucode_t ucode_patch[]={
		{
                        MOVE_DSZ64_DI(TMP1,0x3535),
	...
	hook_match_and_patch(0, 0x0790, 0x7c86);
...
int main(){
	...
	//execute a SYSEXITQ to trigger the patched patch; capture the resulting
