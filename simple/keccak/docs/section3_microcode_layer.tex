...
% hook_match_and_patch()); it then describes how that mechanism is used to
% customise an
...
The x86 instruction set is a contract with software; it is not what the core
executes. A decode unit translates each architectural instruction, which we
...
hardware decoders that emit a handful of $\mu$ops directly; complex or rare
ones are instead decoded by a microcode sequencer, which fetches a stored
$\mu$op program from an on-chip microcode ROM, the MSROM. Goldmont exposes more
...
reverse engineering of Koppe et al.\ on AMD K8/K10 first established this
structure publicly, organising the $\mu$ops into operation classes for
register arithmetic, loads, stores and special operations, and Borrello et al.\
subsequently recovered the corresponding Goldmont repertoire and its operand
...
A triad is thus at once a packing unit, since three $\mu$ops travel together,
and the granularity of control flow, since branches target triads rather than
individual $\mu$ops. The MSROM on Goldmont holds on the order of eight thousand
such triads, which is the program that decodes the entire instruction set.
...
Two lines of prior work make this practical on Goldmont, and our toolchain rests
on both. Borrello et al.'s CustomProcessingUnit recovered the patch and hook
format together with the $\mu$op semantics, and provides an assembler from
$\mu$op mnemonics to triads. Krog's \emph{lib-micro} provides the infrastructure
we build on directly: an eBPF-inspired C macro language for chaining $\mu$ops
into triads, together with a C API that installs a patch and programs a match
entry on an individual core by writing the sequencer arrays through the
undocumented \texttt{udbgrd} and \texttt{udbgwr} debug instructions, rather than
through the signed boot-time update path. Reaching that debug interface requires
a part placed in Intel's debug-unlock state, which we obtain on the
\hbox{Celeron~N3350} through the Intel-SA-00086 Management Engine vulnerability.
Where this body of work used such access to study or to harden the CPU, we use
it to make a cryptographic kernel the body of a hooked instruction, and the rest
of the paper is concerned with doing so well. The two ways in which a primitive
