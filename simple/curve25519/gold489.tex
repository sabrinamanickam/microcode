...
Features of microcode have also been shown to be useful for other purposes, such as security and custom instructions.
...
\paragraph{Reusing $D$ Registers during $\chi$.}
\label{sec:chireuse}
...
%All results come from the Intel Celeron N3350 with the Goldmont core described in \Cref{sec:methodology}. 
%Turbo remains disabled, the processor stays pinned to its base
%frequency, and a single process times every implementation.
%The native implementations build under the six GCC and Clang releases and four optimisation levels listed in \Cref{sec:methodology}.
...
\Cref{sec:methodology} describes the remaining asymmetry in that region. \Cref{tab:results-keccak} reports the results.
...
We then integrate the two patches into a complete Curve25519 implementation and measure the resulting scalar multiplication against native implementations.

\subsection{Challenge and Prior Techniques}
\label{sec:c25519-challenge}

Curve25519 performs most of its computation in field multiplication and squaring modulo $p=2^{255}-19$~\cite{PKC:Bernstein06}.
The Montgomery ladder requires five general field multiplications and four field squarings in each of its 255 iterations.
The final inversion adds another 254 squarings and 11 multiplications before one multiplication converts the projective result to an affine coordinate.
These repeated field operations make multiplication and squaring the principal targets for acceleration.
...
Goldmont provides neither AVX nor the BMI2 and ADX extensions used by these later x86 implementations.
An instruction sequence cannot retain several pending carries in architectural state at the same time.
Architectural code on our target therefore has only the conventional carry flag for \code{ADC}.

Goldmont microcode exposes a different carry mechanism.
Arithmetic $\mu$ops retain conditional state with their destination, which allows a patch to preserve carries from several accumulation chains independently.
This differs from architectural Goldmont code, where conventional \code{ADC} consumes the single carry state in \code{RFLAGS}.

The resulting challenge is to map established Curve25519 arithmetic to this different carry mechanism while keeping the resulting kernels within the available patch RAM.

\subsection{Strategy}
\label{sec:c-strategy}

We place one complete field multiplication and field squaring in their respective microcode patches.
These operations contain enough arithmetic to amortize the approximately seven cycle redirection cost while remaining within the 128 triads available in patch RAM.
A complete Curve25519 ladder step exceeds this capacity.
We therefore retain the scalar loop, conditional swaps, field additions, field subtractions, multiplication by 121665, and the remaining Curve25519 computation in x86 code.

We use the $5\times51$ representation and organize multiplication and squaring around the independent carry values exposed by the microcode interface.
Rather than propagating every carry immediately through \code{RFLAGS}, the patch materializes carries into registers and combines them later.
The surrounding x86 code uses fixed register interfaces and passes intermediate values directly between operations when their register assignments permit it.

We also implement a saturated $4\times64$ microcode control based on \texttt{amd64-64}.
Comparing the two designs lets us determine whether patch execution alone provides the benefit or whether the field representation must also match the carry mechanism exposed by microcode.
...
Goldmont associates conditional state with the destination of an arithmetic $\mu$op.
For an \code{ADD}, this state records whether the addition produced a carry.
Our $5\times51$ implementation uses \code{SETCC\_CONDB} to materialize this carry as an integer equal to zero or one.
The patch can then accumulate the carry in an ordinary register with a subsequent \code{ADD}.
Carries from different additions can therefore remain in separate registers without passing through \code{RFLAGS}.

A microcode \code{ADC} uses a different path.
It consumes the architectural carry flag in \code{RFLAGS} and cannot directly consume the conditional state associated with an arithmetic destination.
To construct a conventional carry chain, the patch first applies \code{GENARITHFLAGS} to the preceding destination and then consumes the resulting carry flag with \code{ADC}.

We use these two mechanisms according to the field representation.
The main $5\times51$ implementation materializes carries with \code{SETCC\_CONDB} and accumulates them explicitly.
The saturated $4\times64$ control instead uses \code{GENARITHFLAGS} and \code{ADC} to propagate overflow across full width limbs.

\subsubsection{Field Multiplication}
\label{sec:c25519-mul}

A $5\times51$ field multiplication forms 25 limb products that contribute to five output limbs.
We accumulate these products without constructing a carry chain through \code{RFLAGS}.

Each 64 bit multiplication produces a low and a high half.
For each output limb, the patch maintains separate accumulators for the low halves, the high halves, and the carries generated while accumulating the low halves.
After an addition to the low accumulator, \code{SETCC\_CONDB} materializes its carry and a later \code{ADD} incorporates that value into the carry accumulator.
The three quantities therefore progress along separate dependency chains.

This organization also permits the multiplication and accumulation work to overlap across consecutive triads.
The following excerpt shows the corresponding schedule.

\begin{lstlisting}[caption={Two triads from the multiplication schedule. The patch materializes carries as register values instead of propagating them through \code{RFLAGS}.},label={lst:c25519-mac}]
{ ADD(TMP0, TMP0, RDX),
SETCC_CONDB(TMP15, TMP0),
ZEROEXT(RDX, R10) }
...
The first triad accumulates the low half of one product, materializes the resulting carry, and prepares an operand for the next multiplication.
The second accumulates a high half and the materialized carry while issuing that multiplication.
The sequential semantics within a triad established in \Cref{sec:background} permit this packing without additional register moves.

We repeat this schedule for the products contributing to each output limb.
After completing one limb, the patch propagates its radix $2^{51}$ carry into the next.
The thirteen unused bits in each limb provide sufficient headroom for this accumulation and allow the propagation to use ordinary register operations rather than a full width \code{ADC} chain.
After the fifth limb, the patch folds the remaining carry into the first limb according to the reduction modulo $2^{255}-19$ described in \Cref{sec:c25519-challenge}.

The complete multiplication occupies 66 triads.
All partial products, accumulators, and carry values remain in registers throughout the patch, so a firing performs no intermediate spills to memory.

\subsubsection{Dedicated Squaring}
\label{sec:c25519-square}

We implement squaring with a separate patch rather than calling multiplication with equal operands.
The $5\times5$ product matrix is symmetric when both operands are equal, which reduces the 25 products required by general multiplication to 15 distinct products.
We account for the symmetric terms by doubling the corresponding operands.

The wrapper prepares the required doubled limbs and multiples by 19 in architectural registers before entering the patch.
Squaring then uses the same low, high, and carry accumulators as multiplication, followed by the same radix $2^{51}$ carry propagation and final reduction.

The reduced product count lowers the patch size from 66 to 42 triads.
The first four output limbs require eight triads each, the fifth requires seven, and three further triads perform the final reduction.
Multiplication and squaring therefore occupy 108 triads together and fit simultaneously within the 128 triads of patch RAM.

We bind multiplication to a repurposed \code{VMWRITE} path and squaring to a repurposed \code{VMREAD} path.
The surrounding X25519 code can consequently invoke either kernel while both patches remain installed.

\subsubsection{Register Chaining Across Field Operations}
\label{sec:c25519-integration}

Invoking the patched kernels through a conventional function interface would reload all five input limbs before every multiplication or squaring.
We avoid these loads when the output of one field operation is consumed immediately by the next.

We implement each Montgomery ladder step in a single inline assembly block and use \code{RBP} to address a fixed layout containing the ladder state.
The addition and subtraction helpers place their results in the five registers expected by the following patched operation.

Within a ladder step, we pass $A=x_2+z_2$ directly to $AA=A^2$ and $B=x_2-z_2$ directly to $BB=B^2$.
The values $D=x_3-z_3$ and $C=x_3+z_3$ similarly remain in registers for $DA=D\cdot A$ and $CB=C\cdot B$.
We also retain $DA+CB$ and $DA-CB$ for their subsequent squarings, and retain $AA+121665E$ for the multiplication that produces the new $z_2$.
Each of these chains removes five input loads while preserving stores required by later consumers.

We apply the same approach to the final inversion.
The fixed addition chain for $z^{p-2}$ contains runs of 2, 5, 10, 20, 10, 50, 100, 50, and 5 consecutive squarings.
Within each run, the output of one squaring becomes the input to the next without first returning the complete field element to memory.
One output limb already occupies its required input register, and the wrapper renames the remaining four registers before invoking the patch again.
Only the boundaries of each repeated squaring run require the field element to be transferred through memory.

\paragraph{Saturated control.}
For comparison, we implement the $4\times64$ arithmetic used by \texttt{amd64-64} with a conventional full width carry chain.
After an arithmetic $\mu$op produces destination associated conditional state, \code{GENARITHFLAGS} transfers that state into \code{RFLAGS} and the following \code{ADC} consumes the carry.
Repeating this sequence propagates overflow across the four saturated limbs.

The resulting multiplication occupies 75 triads.
The control uses the same multiplication kernel for squaring rather than a separate squaring patch.
This implementation provides a comparison with the $5\times51$ design in which the field representation and the microcode carry mechanism are organized differently.

\subsubsection{Verification}
\label{sec:c25519-verification}

We verify the patched field kernels independently before integrating them into X25519.
For multiplication and squaring, the test harness compares each microcode result with both a schoolbook reference and a separate native \code{\_\_uint128\_t} implementation.
The tests cover fixed boundary cases, 10,000 random inputs, and an iterated chain of 1000 operations.

We then verify complete scalar multiplication against the RFC~7748 X25519 test vectors, including the iterated one and 1000 operation tests.
We apply the same tests to the handwritten C, Fiat Cryptography, CryptOpt, \texttt{amd64-51}, and \texttt{amd64-64} implementations used in the evaluation.
These tests exercise scalar clamping, all 255 ladder iterations, inversion, final reduction, and encoding in addition to the patched multiplication and squaring kernels.

\subsection{Performance Evaluation}
\label{sec:c-perf}

We evaluate the Curve25519 implementation at four levels.
We first measure our multiplication and squaring kernels in isolation and the cost of entering patch RAM.
We then replace only the field arithmetic within a common X25519 implementation to determine whether the kernel advantage persists in the complete computation.
We repeat the controlled replacement for a saturated $4\times64$ representation to determine whether the benefit follows from microcode execution alone.
Finally, we compare complete X25519 implementations.

\subsubsection{Field Kernel Performance}
\label{sec:c25519-kernel-results}

We first measure multiplication and squaring independently of the Montgomery ladder.
The benchmark forms a dependent memory to memory chain in which each operation consumes the result of the preceding operation.
Every backend therefore pays the same memory round trip.
\Cref{tab:res-c25519-kernels} reports the median cycles per field operation.

\begin{table}[htbp]
\centering
\caption{Cost of $5\times51$ field multiplication and squaring on the Goldmont N3350.
Measurements form a dependent memory to memory chain so that every backend pays the same memory round trip.
Values are median cycles per field operation.}
\label{tab:res-c25519-kernels}
\footnotesize
\setlength{\tabcolsep}{5pt}
\begin{tabular}{@{}lrr@{}}
\toprule
Backend & \code{fe\_mul} & \code{fe\_sq} \\
\midrule
\textbf{microcode} &
\textbf{122.9} &
\textbf{81.1} \\
...
151.7 &
129.6 \\
hand written \code{\_\_uint128\_t} C &
145.7 &
114.8 \\
\bottomrule
\end{tabular}
...
The microcode kernels have the lowest cost for both operations.
Multiplication requires 122.9 cycles, giving speedups from $1.098\times$ over fiat-crypto to $1.234\times$ over the \texttt{amd64-51} assembly.
Squaring requires 81.1 cycles and provides a larger advantage, with speedups from $1.416\times$ over the hand written C implementation to $1.598\times$ over the \texttt{amd64-51} assembly.
Invoking either patched kernel costs approximately 16 cycles, including operand marshalling, the hooked instruction, redirection into patch RAM, and return.
A trigger-only measurement without operand marshalling requires approximately 7 cycles, with the remaining cost arising from the calling convention.
This invocation cost is included in the reported 122.9 cycle multiplication and 81.1 cycle squaring measurements.

\subsubsection{Controlled Field Arithmetic Comparison}
\label{sec:c25519-field-results}

We measure the contribution of the $5\times51$ microcode kernels within a common X25519 implementation.
All contenders in \Cref{tab:res-c25519-field} use the same C Montgomery ladder, scalar loop, conditional swaps, inversion, and encoding.
Only field multiplication and squaring change between rows.
The comparison therefore isolates the effect of the field backend.

\begin{table}[htbp]
\centering
\caption{Controlled comparison of $5\times51$ field arithmetic.
Every row uses the same C X25519 implementation and differs only in multiplication and squaring.
Best median reports the lowest median obtained by each backend across the 24 compiler and optimization configurations.
Matched speedup is the geometric mean of the per configuration ratio between the native backend and microcode.
A value above one indicates that microcode is faster.}
\label{tab:res-c25519-field}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lrrr@{}}
\toprule
Field backend & Best median & Matched speedup & Wins \\
\midrule
\textbf{microcode $5\times51$} &
\textbf{307,358} &
\textbf{1.000$\times$} &
--- \\
\midrule
...
The microcode backend outperforms every ISA level backend in every matched configuration.
The geometric mean speedup ranges from $1.195\times$ against fiat-crypto to $1.241\times$ against the hand written C implementation.
Against the \texttt{amd64-51} field assembly, which uses the same $5\times51$ representation, microcode achieves a $1.220\times$ geometric mean speedup.

The best absolute medians support the same result.
The microcode backend requires 307,358 cycles on the common ladder.
The best fiat-crypto configuration requires 355,283 cycles, while CryptOpt, the \texttt{amd64-51} field assembly, and the hand written C backend require between 381,025 and 383,608 cycles.
We do not use these independently selected best configurations to attribute the speedup.
The matched comparison provides that attribution.

The kernel and complete computation measurements provide complementary evidence.
In isolation, both patched kernels outperform all four native alternatives under dependent and independent execution.
When integrated into the common X25519 implementation, the same field backend remains faster across all 24 matched compiler and optimization configurations.

This result connects to the carry organization of \Cref{sec:c25519-carries}.
The $5\times51$ representation permits the patch to materialize carries as register values and to maintain the low halves, high halves, and carry counts as separate dependency chains.
The controlled replacement shows that this organization remains advantageous when the kernels execute as part of the complete field computation.

\subsubsection{Effect of the Field Representation}
\label{sec:c25519-representation-results}

The preceding experiment establishes an advantage for $5\times51$, but does not show that executing arithmetic in patch RAM is sufficient to obtain that advantage.
We therefore repeat the controlled field replacement for the saturated $4\times64$ representation used by \texttt{amd64-64}.

Within each representation in \Cref{tab:res-c25519-representation}, the native and microcode variants use the same C ladder and surrounding framework.
The comparison therefore changes only the multiplication and squaring backend.

\begin{table}[htbp]
\centering
\caption{Controlled field backend replacement for the unsaturated $5\times51$ and saturated $4\times64$ representations.
Within each representation, the native and microcode variants use the same ladder and surrounding framework.
Matched ratio is the geometric mean of native cycles divided by microcode cycles across the 24 compiler and optimization configurations.
A value above one indicates that microcode is faster.}
\label{tab:res-c25519-representation}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lrrrl@{}}
\toprule
Representation & Native & Microcode & Matched ratio & Microcode kernels \\
\midrule
$5\times51$ &
387,458 &
329,131 &
\textbf{1.166$\times$} &
66 triad mul, 42 triad sq \\
$4\times64$ &
306,275 &
514,891 &
\textbf{0.595$\times$} &
75 triad mul, sq as mul \\
\bottomrule
\end{tabular}
\end{table}

The two representations produce opposite results.
For $5\times51$, replacing the native field arithmetic with microcode improves performance by a geometric mean factor of $1.166\times$ while the surrounding implementation remains fixed.
For $4\times64$, the same replacement reduces performance to $0.595\times$ that of the native backend.
Equivalently, the saturated microcode field arithmetic is $1.680\times$ slower than the native arithmetic it replaces.

This reversal shows that patch execution alone does not account for the $5\times51$ result.
The unsaturated representation leaves thirteen unused bits in each limb and permits the implementation to accumulate explicit carry values before radix propagation.
The saturated representation provides no corresponding headroom.
Overflow instead propagates across full width limbs through \code{RFLAGS}, which removes the independent carry structure used by the main implementation.

Patch capacity introduces a second constraint.
The $5\times51$ multiplication and dedicated squaring kernels occupy 66 and 42 triads and coexist within the 128 triads of patch RAM.
The saturated multiplier occupies 75 triads.
The remaining capacity is insufficient for a specialized squaring kernel, so the $4\times64$ control computes squaring by invoking multiplication with equal operands.

The negative result therefore identifies a boundary of the optimization.
Field arithmetic benefits from the microcode interface when its representation exposes a carry structure that the patch can exploit and when the required specialized kernels fit within patch RAM.

\subsubsection{End to End X25519}
\label{sec:c25519-e2e}

We finally compare complete X25519 implementations.
Unlike the preceding experiments, the contenders in \Cref{tab:res-c25519-e2e} differ in field representation, ladder organization, field arithmetic, inversion, and integration.
The table therefore establishes the overall position of our implementation but does not attribute performance differences to microcode.

\begin{table}[htbp]
\centering
\caption{End to end X25519 on the Goldmont N3350.
Each implementation appears in its best configuration across the 24 compiler and optimization configurations.
Values report median cycles per scalar multiplication.
The contenders differ in representation and implementation structure.}
\label{tab:res-c25519-e2e}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}llr@{}}
\toprule
Implementation & Representation and backend & Median \\
\midrule
amd64-64 asm &
$4\times64$, assembly &
271,756 \\
\textbf{this work} &
\textbf{$5\times51$, microcode} &
\textbf{300,205} \\
amd64-51 framework with microcode &
$5\times51$, microcode &
314,851 \\
\code{donna\_c64} &
$5\times51$, portable C &
337,876 \\
fiat-crypto &
$5\times51$, generated C &
355,283 \\
amd64-51 asm &
$5\times51$, assembly &
356,704 \\
CryptOpt &
$5\times51$, optimized assembly &
381,025 \\
hand written C &
$5\times51$, \code{\_\_uint128\_t} &
381,115 \\
\bottomrule
\end{tabular}
\end{table}

Our complete implementation requires 300,205 cycles and is the fastest $5\times51$ implementation in the comparison.
It requires 15.8\% fewer cycles than the complete \texttt{amd64-51} assembly implementation and 11.2\% fewer cycles than \texttt{donna\_c64}.
It also outperforms the fiat-crypto, CryptOpt, and hand written C implementations.

The saturated \texttt{amd64-64} assembly remains the fastest implementation overall at 271,756 cycles and requires 9.5\% fewer cycles than our implementation.
This result is consistent with the controlled saturated experiment rather than in conflict with the field arithmetic result.
The $4\times64$ representation is precisely the case in which the available microcode carry mechanism and patch capacity do not provide an effective mapping.

Taken together, the experiments identify both the source and the boundary of the Curve25519 result.
At the kernel level, the patched multiplication and squaring operations outperform the native alternatives under both serial dependencies and exposed instruction level parallelism.
Within a controlled X25519 implementation, the resulting $5\times51$ backend outperforms every alternative field backend across all 24 matched configurations.
The saturated control reverses this advantage.
Microcode therefore does not provide a uniform acceleration for multi precision arithmetic.
Its benefit depends on the interaction between the field representation, the available carry mechanism, and the finite patch capacity.
