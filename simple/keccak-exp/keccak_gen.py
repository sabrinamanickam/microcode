...
def gen_body(ops):
    ...
    # === C-RESIDENT theta (keccak-exp EXPERIMENT) ===
    # The 5 column parities C stay in registers (CREG = D_REG) instead of round-
    # tripping through the buffer; this kills the 5 stores (theta-C) + 10 loads
    # (theta-D) per round that the production version pays. theta-C keeps the
    # depth-3 balanced tree (RSP is still free here -> NO critical-path penalty).
    # theta-D computes D in place into CREG; the in-place 5-cycle needs 3 save-
    # temps, so we spill 2 idle state lanes to the buffer (reloaded before the
    # apply) to free 2 registers and use {RSP, SP0, SP1} as the 3 saves.
    # Verified hazard-free 3-save schedule: produce D0, D2, D1, D4, D3.
    CREG = D_REG   # [RAX,TMP12,TMP13,TMP14,TMP15]: holds C[x] in theta-C, D[x] after
    for x in range(5):
        emit(ops,"XOR", CREG[x], REG[x],    REG[x+5])   # a^b
        emit(ops,"XOR", "RSP",   REG[x+10], REG[x+15])  # c^d  (RSP free during theta-C)
        emit(ops,"XOR", CREG[x], CREG[x], "RSP")        # (a^b)^(c^d)
        emit(ops,"XOR", CREG[x], CREG[x], REG[x+20])    # ^e   -> depth 3 (no penalty)
    # spill 2 idle state lanes -> 2 free temps for the in-place D 5-cycle
    SP0, SP1 = REG[23], REG[24]          # lanes 23,24 (TMP10,TMP11); reloaded pre-apply
    emit(ops,"ST", SP0, OFF(DSCR+0))
    emit(ops,"ST", SP1, OFF(DSCR+1))
    t0, t1, t2 = "RSP", SP0, SP1         # the 3 save-temps
    emit(ops,"MOV", t0, CREG[0])         # save C0
    emit(ops,"MOV", t1, CREG[2])         # save C2
    emit(ops,"MOV", t2, CREG[4])         # save C4
    emit(ops,"ROL", CREG[0], CREG[1], 1); emit(ops,"XOR", CREG[0], CREG[0], t2)      # D0 = rol(C1)^C4
    emit(ops,"ROL", CREG[2], CREG[3], 1); emit(ops,"XOR", CREG[2], CREG[2], CREG[1]) # D2 = rol(C3)^C1
    emit(ops,"ROL", CREG[1], t1, 1);      emit(ops,"XOR", CREG[1], CREG[1], t0)      # D1 = rol(C2)^C0
    emit(ops,"ROL", CREG[4], t0, 1);      emit(ops,"XOR", CREG[4], CREG[4], CREG[3]) # D4 = rol(C0)^C3
    emit(ops,"ROL", CREG[3], t2, 1);      emit(ops,"XOR", CREG[3], CREG[3], t1)      # D3 = rol(C4)^C2
    emit(ops,"LD", SP0, OFF(DSCR+0))     # reload the 2 spilled state lanes before the apply
    emit(ops,"LD", SP1, OFF(DSCR+1))
