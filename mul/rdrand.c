...
void install_addshl_vmwrite_patch(void) {
    ucode_t ucode_patch[] = {
        {         
          ZEROEXT_DSZ64_DI(RBX,0x9090),
          //MOVE_DSZ64_DI(RBX,0x9090),
          NOP,
          NOP,
          END_SEQWORD
        }
    ...
    hook_match_and_patch(0 /*core*/, 0x0428 /*vmwrite_r64_r64_xlat*/, 0x7c4c /*slot*/);
}
int main(void)
{ 
...
/*  printf("Match-and-patch table:\n");
for (int i = 0; i < 0x20; i++) {
    uint64_t val = ldat_array_read(0x6a0, 3, 0, 0, i);
    if (val != 0)
        printf("  slot %02x: 0x%016" PRIx64 "\n", i, val);
}*/
  for (int i = 1; i <= 0x1e; i++) {
    ldat_array_write(0x6a0, 3, 0, 0, i, 0);
}

/* then clear IN hook last */
ldat_array_write(0x6a0, 3, 0, 0, 0x1f, 0);
	asm volatile(
		       "rdrand %%rbx\n\t" 
